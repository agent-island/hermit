...
Project AA is an experimental system, so changes must preserve a clear distinction between what the runtime does, what a model says, and what an observer concludes. A model's self-description belongs in the record; it must not silently become a claim made by the software. Likewise, an action description must match the implemented effect exactly. In particular, `email()` is deliberately a local letter-writing simulation and must never be described as sent or delivered.
